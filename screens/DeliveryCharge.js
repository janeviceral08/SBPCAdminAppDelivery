  ...
  render() {
    return (
        ...
        <Header androidStatusBarColor="#2c3e50" style={{display:'none'}} style={{backgroundColor: 'salmon'}}>
         ...
         <Text style={{marginTop: 15, fontSize: 10}}>Rider Commission Charge</Text>
         <Item regular style={{marginTop: 7}}>
...
const dataValue = {
   ...
   del_charge : parseFloat(this.state.amount_base),
 ...
 firestore().collection('charges').doc('delivery_charge').update(dataValue).then((docRef) => {   
      this.setState({
          loading: false,
        });
  this._bootstrapAsync();
